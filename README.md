...
> **2026-09-12 재평가:** 최신 운영 기준은 [기획 재평가·제출 계획](docs/15_restart-plan-20260912.md)이다.
> 기존 제출 기록은 있으나 신규 학습·GPU 재현·점수 개선은 아직 미검증이다.
> 무료 Colab 실행용 `notebooks/colab_verify.ipynb`와 `verify_bundle.zip`을 갱신했다.
> 아래 과거 설계의 실측·최적성 주장은 원본 실행 기록과 구분해서 읽는다.
...
2. 현재 `file_head=direct`에서는 파일 판별과 음악 헤드가 독립이다. 음악 개선이 FILE까지 전파된다고 가정하지 않는다.
3. 존재 탐지는 기존 CPS 기록이 높아 우선 변경을 줄이고 회귀 검사 대상으로 둔다. 최적이라고 확정하지 않는다.
...
**기존 제출 이후 검증 체계 재구축 중.** 공개 데이터 조사·검증 가드 수정·GPU 실행 준비까지 진행했으며, 실제 신규 학습과 성능 평가는 아직 수행하지 않았다.
